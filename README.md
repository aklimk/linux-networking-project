...
# Features
- Uses low-level linux and libc library calls to instantiate and communicate over TCP sockets, to parse arguments and to handle multithreading.
- Server is multi-threaded and allows for mutliple simultanious connected clients. Multithreading uses libc semaphores, mutexes and flags to safely synchronize resources.
- Includes a custom command line argument parser in the client implementation.
- Uses TCP, IPv4 and HTTP style requests for client/server communication.
